...
#現在のセルの数を表示
execute store result score 生きているセルの数 LifeGameDisplay run execute if entity @e[tag=Live]

#必ず最後のコマンドにする
#各種検知スコアをリセット
scoreboard players set Looping LifeGameCore 0
scoreboard players reset @a LifeGameClick
execute as @a run execute unless data entity @s {SelectedItem:{tag:{Loop:1}}} run scoreboard players set warm LifeGameCore 0